...
async def test_create_endpoint_directly_for_cloud_model():
    ...
    mock_model.cloud_model_id = cloud_model_id
    mock_model.status = ModelStatusEnum.ACTIVE
    mock_model.source = "openai"  # For proxy cache
    ...
    with patch("budapp.model_ops.crud.ModelDataManager") as mock_model_manager, patch(
        ...
        endpoint_data = mock_endpoint_manager.return_value.insert_one.call_args[0][0]

        # Verify endpoint data
        assert endpoint_data["project_id"] == project_id
        assert endpoint_data["model_id"] == model_id
        assert endpoint_data["cluster_id"] == cluster_id
        assert endpoint_data["name"] == "test-endpoint"
        assert endpoint_data["status"] == EndpointStatusEnum.RUNNING
        assert endpoint_data["credential_id"] == credential_id
        assert endpoint_data["supported_endpoints"] == ["chat", "completion"]
        assert endpoint_data["active_replicas"] == 1
        assert endpoint_data["total_replicas"] == 1
        assert endpoint_data["number_of_nodes"] == 1
        assert "namespace" in endpoint_data
        assert "url" in endpoint_data
