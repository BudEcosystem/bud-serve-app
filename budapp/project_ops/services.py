...
from ..permissions.schemas import CheckUserResourceScope, PermissionList, RemoveResource, RemoveUserFromResource
...
class ProjectService(SessionMixin):
    ...
    async def retrieve_active_project_details(
        self, project_id: UUID, user_id: UUID
    ) -> Union[Tuple[ProjectModel, int], None]:
    ...
    async def delete_active_project(
        ...
        if is_benchmark:
            ...
            # )
            _ = await PermissionService(self.session).remove_resource(
                RemoveResource(resource_type="endpoint", entity_id=str(project_id))
            )
        ...
        for db_user in db_users:
            ...
            _ = await PermissionService(self.session).delete_permission_for_resource(
                RemoveUserFromResource(
                    resource_type="endpoint", entity_id=str(project_id), user_auth_id=db_user.auth_id
                )
            )
    ...
    async def _get_parsed_project_user_permissions(
        self,
        results: List[Tuple[UserModel, str]],
